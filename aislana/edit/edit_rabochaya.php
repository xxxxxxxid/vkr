...
function addToDatabase() {
  ...
  var kol = $("#kol").val();
    ...
    $.ajax({
    url: "../insert/insert_rabochaya.php",
    type: "POST",
...
$id=(int)$_POST['id'];
$field= $_POST['field'];
$tab=mysqli_query($link, "UPDATE rabochaya SET $field ='$value' WHERE  id=$id");
 ...
 <?while($i<count($tab))
{
 ...
 <tr>
  ...
  <td style='border-left: none;'><?echo "<a href='edit_rabochaya.php?id=$row[aud]#modal' style='margin:0; padding:1.5px;' class='mod' title='Подробно'><img src='../images/dop.png' width= '25px' height= '25px'></a>";?></td>
  <td contentEditable class='edit' data-id='<?echo $row['id']?>' data-name="kol"><?echo $row['kol']?></td>
  <td><?echo"<a href = '../delete/del_rabochaya.php?id=$row[id]' class='ajax-action-links' name = 'del'>Удалить</a>"?></td>
</tr> <?
}
$i++;
}?>
...
<div class="modal" id="modal">
  <div class="modal__content">
    <a href="#" class="modal__close"><center>X</center></a>
          <? $tab1 =  mysqli_query( $link,  "SELECT * FROM dop WHERE aud=$_GET[id] and korpus_id=4");
          $i = 0; $num=1;
?>
