...
function addToDatabase() {
  ...
  var kol = $("#kol").val();
    ...
    $.ajax({
    url: "../insert/insert_lenina_3.php",
    type: "POST",
...
$id=(int)$_POST['id'];
$field= $_POST['field'];
$tab=mysqli_query($link, "UPDATE lenina3 SET $field ='$value' WHERE  id=$id");
...
$tab  =  mysqli_query( $link,  "SELECT * FROM lenina3" );
if(isset($_POST['text'])){
  $tab = mysqli_query( $link, "SELECT * FROM lenina3 WHERE aud = {$_POST['text']}");
}?>
 ...
 <?while($i<count($tab))
{
 ...
 <tr>
  ...
  <td style='border-left: none;'><?echo "<a href='edit_lenina_3.php?id=$row[aud]#modal' style='margin:0; padding:1.5px;' class='mod' title='Подробно'><img src='../images/dop.png' width= '25px' height= '25px'></a>";?></td>
  <td contentEditable class='edit' data-id='<?echo $row['id']?>' data-name="kol"><?echo $row['kol']?></td>
  <td><?echo"<a href = '../delete/del_lenina_3.php?id=$row[id]' class='ajax-action-links' name = 'del'>Удалить</a>"?></td>
</tr> <?
}
$i++;
}?>
